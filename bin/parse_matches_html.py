...
parser.add_argument("--outfile", default="matches_out_005.html", help="Output file")
...
matches_1_file = Path(matches_dir) / f"matches_1_out_{threshold}.csv"
outfile        = args.outfile   # matches_dir / f"matches_out_{threshold}.html"
...
with open(outfile, "w") as o, open(matches_file) as m:
    ...
    # total
    total_new_seqs_ct = (
        ex_seqs_count + new_seqs_count + new_singleton_seqs_count + nc_seqs_count + nc_singleton_seqs_count
    )
    ...
    o.write(
        "<p class='small_text'>NB! Only DNA sequences placed into existing compound clusters are included in the following taxonomy distribution table.</p>\n"
    )
    o.write(
    ...
    for kgd in seq_taxonomy_dict:
        o.write(
            f"<tr><td><b>{kgd}</b></td><td>{seq_taxon_count[kgd]}</td><td>{len(seq_taxon_sh_count[kgd])}</td><td></td></tr>"
        )
        for phy in seq_taxonomy_dict[kgd]:
            tax_name_p = kgd + ";" + phy
            o.write(
                f"<tr><td>&emsp;{phy}</td><td>{seq_taxon_count[tax_name_p]}</td><td>{len(seq_taxon_sh_count[tax_name_p])}</td><td></td></tr>"
            )
            for clss in seq_taxonomy_dict[kgd][phy]:
                tax_name_c = tax_name_p + ";" + clss
                o.write(
                    f"<tr><td>&emsp;&emsp;{clss}</td><td>{seq_taxon_count[tax_name_c]}</td><td>{len(seq_taxon_sh_count[tax_name_c])}</td><td></td></tr>"
                )
                for order in seq_taxonomy_dict[kgd][phy][clss]:
                    tax_name_o = tax_name_c + ";" + order
                    o.write(
                        f"<tr><td>&emsp;&emsp; &emsp;{order}</td><td>{seq_taxon_count[tax_name_o]}</td><td>{len(seq_taxon_sh_count[tax_name_o])}</td><td>"
                    )
                    ...
                    for fam in seq_taxonomy_dict[kgd][phy][clss][order]:
                        ...
                        o.write(
                            f"<tr><td>&emsp;&emsp; &emsp;&emsp;{fam}</td><td>{seq_taxon_count[tax_name_f]}</td><td>{len(seq_taxon_sh_count[tax_name_f])}</td><td></td></tr>"
                        )
                        for gen in seq_taxonomy_dict[kgd][phy][clss][order][fam]:
                            tax_name_g = tax_name_f + ";" + gen
                            o.write(
                                f"<tr><td>&emsp;&emsp; &emsp;&emsp; &emsp;{gen}</td><td>{seq_taxon_count[tax_name_g]}</td><td>{len(seq_taxon_sh_count[tax_name_g])}</td><td></td></tr>"
                            )
                            for spe in seq_taxonomy_dict[kgd][phy][clss][order][fam][gen]:
                                tax_name_s = tax_name_g + ";" + spe
                                o.write(
                                    f"<tr><td>&emsp;&emsp; &emsp;&emsp; &emsp;&emsp;{spe}</td><td>{seq_taxon_count[tax_name_s]}</td><td>{len(seq_taxon_sh_count[tax_name_s])}</td><td>"
                                )
                                for sh in seq_taxon_sh_count[tax_name_s]:
                                    o.write(
                                        f"<a href='https://unite.ut.ee/sh/{sh}' target='_blank'>{sh}</a> ({seq_taxon_sh_count[tax_name_s][sh]}); "
                                    )
                                o.write("</td></tr>")
    o.write("</table>\n")
